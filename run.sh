...
ld -m elf_i386 -T linker.ld -o kernel.elf prog.o kernel.o kheap.o kprintf.o keyboard.o kshell.o timer.o kproc.o proc.o mem.o syscall.o gdt.o disk.o

# Building kernel bin
objcopy -O binary kernel.elf kernel.bin
REAL_KERNEL_SECTORS=$((($(stat -c%s kernel.bin) + 511) / 512))
echo "REAL sectors = ${REAL_KERNEL_SECTORS}"
truncate -s 524288 kernel.bin
KERNEL_SECTORS=$((($(stat -c%s kernel.bin) + 511) / 512))
echo "Kernel sectors = ${KERNEL_SECTORS}"

# Building user programs.
# user
nasm -f bin user/user.asm -o user.bin

# Building disk image
cat boot.bin kernel.bin user.bin >test_disk.img
